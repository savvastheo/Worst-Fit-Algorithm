...
{ 
	...
	{
            ArrayList<Integer> allocationBlocks=new ArrayList<>();
            int totalBlocks=sizeOfBlocks.length;
            int totalProcesses=sizeOfProcesses.length;
            
            for (int i=0;i<totalProcesses;i++){
                allocationBlocks.add(-255);
            }
            for (int i=0;i<totalProcesses;i++){
                int tempIndex=-1;
                for (int j=0;j<totalBlocks;j++){
                    if (sizeOfBlocks[j]>=sizeOfProcesses[i]){
                        if (tempIndex==-1){
                            ...
                            if (sizeOfBlocks[j]>sizeOfBlocks[tempIndex]){
                                tempIndex=j;
                            }
                        }
                    }
                ...
                if (tempIndex!=-1){
                    allocationBlocks.set(i, tempIndex);
                    sizeOfBlocks[tempIndex]-=sizeOfProcesses[i];
                }
            }
